...
## Bug fix, round 2 — account picker checkmark overlapping balance digits (2026-09-17, real device report, after the round-1 positioning fix)

**Trigger**: user screenshot of the account picker dropdown, post-deploy of this session's earlier `alignItemWithTrigger` positioning fix — the popup now floats correctly, but the checkmark (✓) on the currently-selected item visually overlapped the tail end of its balance text ("...20.00"), and a second account's institution name visibly extended into the space where an unrelated field's placeholder text sat behind the popup.

**Root cause, found via reproduction with real-shaped data (2 accounts, one with a long institution name), not guessed**: this is the exact `min-w-0` flexbox-overflow defect class already fixed ~14 times earlier this session in card components (carried-forward lesson #14) — but the actual blocker was one layer deeper than `account-picker.tsx`'s own markup. The shared `SelectItem`'s `ItemText` wrapper (`src/components/ui/select.tsx`) had `shrink-0` — which unconditionally disables flex-shrink, so **no `min-w-0` added anywhere inside it could have fixed this**; `flex-shrink:0` overrides regardless of `min-width`. With a two-part row (name+institution on one side, a `shrink-0` balance on the other) inside that non-shrinking wrapper, the row simply overflowed the item's padded content box in real pixels, running the balance text directly under the checkmark's `absolute right-2` position (measured and confirmed via bounding boxes before/after each change, not assumed).

**Fix, in three parts, each verified in turn before moving to the next**:
1. `account-picker.tsx`: added `min-w-0`/`truncate` to the name+institution wrapper and its text (the locally-obvious fix — necessary but, on its own, not sufficient, confirmed by re-measuring and seeing zero change).
2. `select.tsx`: removed `shrink-0` from the shared `ItemText`, added `min-w-0` instead (`flex min-w-0 flex-1 gap-2 whitespace-nowrap`) — this is what actually let the row shrink at all. Checked all 16 other files using `SelectItem` first: every other one renders a single short plain-text label with no competing fixed-width sibling, so allowing them to shrink (which only matters when content doesn't already fit) changes nothing for them — confirmed by the full build/test pass and by the fact none of their content is close to the item's `min-w-36` (144px) floor.
3. Widening the popup itself for this specific picker (`className="min-w-64"` on `AccountPicker`'s `<SelectContent>`) — with the shrink fix alone, the shared component's `min-w-36` (144px) floor was still too narrow to show icon+name+balance without illegibly truncating even a 4-character name ("Cash" rendered as a single stray character before this). 144px is fine for every other Select in the app (single short labels); this one specifically needed more room for its two-part row.

**Verified visually at each of the 3 steps** (not just once at the end) using the same temporary-preview-page-plus-Playwright-bounding-box technique as this session's other fixes — measuring the balance text's and checkmark's actual pixel rectangles to confirm zero overlap, then a final screenshot confirming both accounts render fully legible with no truncation, no overlap, and the correct checkmark on the selected one. Preview route and spec deleted afterward.

Quality gate: lint ✅, typecheck ✅, tests ✅ (509/509, unchanged), build ✅ (49 routes).

## Feature — tap a transaction row to reveal its note inline (2026-09-17, user request)

**Trigger**: the user pointed at a transaction in the dashboard's "รายการล่าสุด" (Recent transactions) list, then showed the same transaction's edit sheet with a saved note ("ไปทำงาน") and asked for tapping the row to slide out and show that detail, instead of only being reachable by opening the full edit form via the "⋮" menu.

**What existed before**: `TransactionRow` (`src/features/transactions/components/transaction-row.tsx`, shared by both the dashboard's compact list and the full `/money/transactions` list — one fix covers both) had no tap behavior on the row itself at all; the only way to see a transaction's note was "⋮" → Edit, opening the full amount/category/account/date edit sheet just to peek at a note.

**Added**: tapping the row's icon+title+account/date area now expands a small inline panel below it showing the note, with a chevron that rotates to indicate state — matching the existing expand/collapse pattern already used by the dashboard's Wealth Score and Safe-to-Spend cards (`fade-in slide-in-from-top-1 duration-(--motion-normal)`, `rotate-180` chevron), so this isn't a new interaction pattern for the app, just applied to a third place. The "⋮" menu (Edit/Delete) is unchanged and still works independently.

**Deliberately conditional — no dead interaction**: the row is only made tappable (and only shows the chevron) when `transaction.notes` actually has content. Per CLAUDE.md's "no dead buttons" rule, a transaction with no note doesn't get an expand affordance that would open to reveal nothing — verified this explicitly with two preview rows side by side (one with a note, one without) rather than assuming.

**Verified visually**: same temporary-preview-page-plus-Playwright-screenshot technique as this session's other fixes, using the exact reported data (merchant "Grab", ฿150 expense, note "ไปทำงาน") — confirmed closed state shows the chevron only on the row with a note, and tapping expands the note text with the chevron flipped, both screenshotted. Preview route and spec deleted afterward.

Quality gate: lint ✅, typecheck ✅, tests ✅ (509/509, unchanged — no domain/financial logic touched), build ✅ (49 routes).

## Fix — AI chat force-scrolled to the bottom on every streamed token, trapping the reader (2026-09-17, real device report)

**Trigger**: the user tried the live AI chat and reported that while a reply is streaming in, the page keeps snapping back to the bottom on every new chunk, so scrolling up to reread an earlier part of the answer while it's still arriving is effectively impossible — they asked for it to follow "a little" but let the user scroll freely mid-stream.

**Root cause**: `AICoachChat`'s auto-scroll effect (`src/features/ai/components/ai-coach-chat.tsx`) ran `bottomRef.current?.scrollIntoView()` on every change to `messages` — and during streaming, `messages` changes on every single delta chunk from the server (potentially dozens of times per reply), so the page was yanked to the bottom dozens of times per answer with no way to opt out.

**Fix**: classic chat-app "sticky scroll" pattern. A `stickToBottomRef` tracks whether the user is currently within ~120px of the bottom of the page (updated via a `window` scroll listener); the auto-scroll effect only fires when that's true. The moment the user scrolls away during a reply, following stops — the rest of the stream renders in place without moving their viewport. Sending a new message re-arms sticky-scroll (matches the ChatGPT-style convention: your own action re-anchors you to the conversation; someone else's/the assistant's continued output doesn't drag you along).

**Verified with a real simulated stream, not just code review** — this class of bug is notoriously easy to get subtly wrong (timing, listener passivity, effect ordering), so it was reproduced properly: a temporary preview page seeded 8 prior exchanges (enough to make the page genuinely scrollable) plus a temporary `__qaSimulateStream` escape hatch on the real component that appends words on an interval, mimicking real SSE deltas. Playwright scripted: let streaming run briefly, scroll away to a specific position, let streaming continue for another second-plus, and assert the scroll position barely moved (**confirmed: stayed at exactly the same pixel** while the simulated reply kept growing below) — then sent a new message and confirmed the page correctly snapped back to the true bottom afterward (**confirmed: landed exactly at `document.documentElement.scrollHeight`**). Both the seeding prop and the stream-simulation effect were fully removed from the real component afterward, along with the preview route and its spec — `git status` shows zero trace.

Quality gate: lint ✅, typecheck ✅, tests ✅ (509/509, unchanged), build ✅ (49 routes).

## Billing audit + two real Pro-exclusive features (2026-09-17)

**Trigger**: the user asked whether the plan tiers had actually been verified — that whoever pays for a given level really gets the features promised and that each is genuinely enforced, not just displayed. Audited every `FEATURES`/limit entry against its real code path (not just `plans.ts`'s config) before answering:

| Claim | Verified enforcement point |
|---|---|
| Free: max 3 active goals | `goals/actions.ts` — server-side count check before insert |
| Plus+: Forecast / Debt Planner / Monthly Review / Subscription Detector | Each page's own `requireFeature()` gate runs **before** any real data is fetched — a denied request never even queries the gated data |
| Free: 3 Income Opportunities / Missions | `opportunity-list.tsx` / `mission-list.tsx` — real `.slice()`, with a "N more in Plus" card for the remainder |
| 15 / 150 / 500 AI messages per month | `/api/ai/chat/route.ts` checks quota **before** calling the provider (a blocked request costs nothing) |

**Found one real gap**: Pro's pricing bullet "สิทธิ์ใช้ฟีเจอร์ใหม่ก่อนใคร" (early access to new features) had **zero mechanism behind it anywhere in the codebase** — pure marketing copy with nothing to actually deliver on it. Also found, while tracing this: `ADVANCED_INSIGHTS` was set `true` for **both** Plus and Pro, and was never read by any code path at all — meaning Pro was, in practice, a strict feature-identical copy of Plus with only a higher AI message cap, exactly matching a "Known Limitation" this file had already flagged in Day 7 but never closed.

**User asked for two real, working Pro-exclusive features to replace the unbacked bullet — both built and verified this session:**

1. **All AI Insights, not just the top one** (`ADVANCED_INSIGHTS`, now genuinely Pro-only — changed `plus.features.ADVANCED_INSIGHTS` from `true` to `false`). `src/features/ai/lib/insights.ts` gained `getVisibleInsights()`: Free/Plus still see exactly one (same as before, via the unchanged `getTopInsight()`), Pro sees every currently-meaningful insight from the same already-deterministic `buildInsights()` — no new insight logic, purely how much of the existing output is shown. Wired into `/ai` page only (the dashboard's compact insight slot deliberately stays single-insight for every plan, preserving its "don't crowd the dashboard" design intent from Day 6 — the differentiation shows up on the dedicated AI page instead).
2. **CSV export of transaction history** (new `DATA_EXPORT` feature, Pro-only). `src/features/transactions/export.ts` — a pure, dependency-free `transactionsToCsv()` formatter (raw decimal amounts, never ฿-formatted display strings, so a spreadsheet can sum/chart directly; proper CSV quoting/escaping for commas and embedded quotes; transfers render as "From → To"). `GET /api/export/transactions` gates with the same `requireFeature()` pattern as every other premium route, fetches the user's own RLS-scoped transactions, and streams back a UTF-8-BOM-prefixed CSV file (the BOM specifically so Excel — still the most common consumer — doesn't mangle Thai text into question marks) with a `Content-Disposition: attachment` header. A new `ExportTransactionsButton` (plain server-rendered link, no client JS needed) sits next to the existing filter bar on `/money/transactions`: a real download link for Pro, a real link to `/pricing` for everyone else — never a disabled dead button either way.

Pricing copy updated in both locales: Pro's bullet list now reads the two real features instead of the unbacked "early access" claim.

**Verified**: 7 new unit tests for `transactionsToCsv` (header/row shape, raw-decimal-not-formatted amounts, locale-aware category names, transfer "from → to" formatting, comma/quote CSV-escaping, empty-list edge case) and 3 new tests for `getVisibleInsights` (Free/Plus capped at 1, Pro sees all, empty-array — not null — when nothing qualifies). Existing `tests/billing-plans.test.ts` had one test genuinely wrong under the new design ("Plus unlocks every premium feature") — fixed to correctly assert Plus gets every *Plus-tier* feature while explicitly NOT getting the two Pro-exclusives; added a dedicated test asserting the Free/Plus/Pro split for both new flags precisely. Visually confirmed via a temporary preview (deleted after): the new Pro bullets render cleanly with no overflow, and the export button correctly shows its locked (lock icon, links to `/pricing`) state for an unauthenticated/Free request.

Quality gate: lint ✅, typecheck ✅, tests ✅ (520/520 — 500 prior + 7 export + 3 insights, plus 2 billing-plans tests rewritten), build ✅ (49→50 routes, new `/api/export/transactions`).

**Not yet verified live**: an actual authenticated Pro-plan request downloading a real CSV end-to-end (would need a real Pro session; the route's logic mirrors `/api/ai/chat`'s already-live-verified gate pattern exactly, and the formatter itself is unit-tested against realistic data, but the full HTTP round trip with a real browser download hasn't been exercised).

## GoalIllustration — abstract "moving toward the goal" motion, not a character (2026-09-17)

**Trigger**: the user liked the existing Goals empty-state illustration (flag + dashed path) but asked for it to feel like "a kid chasing their dream of being rich, walking toward the flag."

**Conflict flagged before building anything**: a literal character/mascot directly contradicts GRAPHICS_PLAN.md's explicit rule — "never detailed character art or realistic human figures, which read as either childish or expensive stock art" and "a mascot-first product" is listed under "what WEALTH OS is not" (the same reasoning the AI Coach illustration was deliberately built around, avoiding a robot/mascot). Per CLAUDE.md's own precedence rule (the documented visual-identity owner wins over a literal feature request when the two conflict), this wasn't implemented as literally asked — the conflict was explained to the user with the exact documented reasoning, and three concrete options offered (abstract motion cue / a maximally-reduced abstract dot-figure / knowingly override the brand rule). **User picked the abstract-motion option.**

**What shipped**: a small solid dot now travels the exact same dashed path the illustration already had, from its start toward the flag, on a slow (3.2s) calm loop — fading in at the path's start and out just before the flag so the loop restarts invisibly instead of visibly snapping back. Implemented as a real CSS `@keyframes` animation using `offset-path`/`offset-distance` (`.motion-goal-dot` in `globals.css`), specifically so it's automatically caught by the existing Day 8 `prefers-reduced-motion` blanket rule with zero extra code — no separate reduced-motion handling was needed, confirmed by reading that rule's `*`-selector scope before choosing this approach over an SVG-native `<animateMotion>` (which would *not* have been automatically covered). No character, no face, no color outside the existing palette — purely an abstract "progress in motion" cue.

**Verified visually, not just by reading the CSS**: a temporary preview page + Playwright, screenshotted at three points across one animation cycle — confirmed the dot visibly at different positions along the curved path each time (not stuck, not jumping), computed `getComputedStyle().offsetPath`/`offsetDistance` matched the intended path and a real mid-animation percentage, and zero console errors. Preview route and spec deleted afterward.

Quality gate: lint ✅, typecheck ✅, tests ✅ (520/520, unchanged — pure CSS/illustration change), build ✅ (49 routes).

## Income Target date — duration-preset chips + a visual timeline bar (2026-09-17)

**Trigger**: the user's screenshot showed the Income Target form's "target date" field using a plain native `<input type="date">` (a full calendar grid popup) — asked for quick preset buttons (6 months, 1 year, 5 years, "or whatever makes sense"), plus something visual ("หลอดเส้น") showing when the target date actually lands.

**Design reused, not invented**: this exact "preset chips + custom option" pattern already existed in the app (`emergency-fund-form.tsx`'s month-count chips, built Day 2) — followed it closely for consistency rather than building a new interaction style. `src/features/income-target/components/target-date-field.tsx` (new): five duration presets (3mo/6mo/1yr/2yr/5yr) as pill buttons, computing `today + N months` via a new shared `addMonthsClamped()` helper (`src/lib/date.ts`) — correctly clamped for month-end overflow (Jan 31 + 1 month → Feb 28/29, never a silent roll into March), the exact same clamping rule `calculateNextDueDate()` already used for recurring transactions but not previously exposed as a generic, reusable utility. A "กำหนดเอง" (custom) chip reveals the original native date input for anyone who wants an exact date instead of a preset. The visual "หลอดเส้น" the user asked for: a horizontal bar from "วันนี้" (today) to the resolved date, plus a plain-language line above it ("จะถึงเป้าหมายในอีกประมาณ N เดือน") — an approximate month count for the custom-date case (day-difference ÷ 30.44, clearly a "ประมาณ/about" figure, never presented as exact).

Still submits under the same `target_date` form field name/format as before, so `saveIncomeTarget`'s existing validation/action needed zero changes — purely a presentation-layer replacement.

**Verified**: 5 new unit tests for `addMonthsClamped` (simple case, the 5-year multi-year span, month-end clamping into both a non-leap and a leap February, a zero-month no-op) — all pass. Visually confirmed via a temporary preview + Playwright across all three interaction states: default (1-year preset selected, bar + "12 เดือน" label correct), switching to the 5-year preset (bar/label update to "60 เดือน" and the correct future date), and switching to "กำหนดเอง" (native date input appears, the bar correctly disappears until a date is actually picked, rather than showing a stale/wrong one). Preview route and spec deleted afterward.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525 — 520 prior + 5 new date tests), build ✅ (49 routes).

## Income Target — foldable view/edit pattern (2026-09-17)

**Trigger**: after using the new duration-preset field above, the user saved an Income Target and reported the form just stayed open with the saved values still sitting in the inputs — no confirmation the save actually "landed" — and asked whether there could be a foldable pattern instead: a collapsed view that opens into the editable form on demand.

**Root cause**: `IncomeTargetSection` rendered `IncomeTargetForm` directly and unconditionally, with no concept of a "saved, now just show it" state — every other comparable feature in the app (`EmergencyFundView` being the closest sibling: same shape, a single optional row the user sets up once and revisits occasionally) already solves this with a view/edit toggle, but Income Target had never been given the same treatment when it was first built.

**Fix**: followed `EmergencyFundView`'s exact existing pattern rather than inventing a new one. New `IncomeTargetView` (`src/features/income-target/components/income-target-view.tsx`) owns the toggle: no target yet → form starts open (nothing to fold, matches every other first-time-setup case in the app); an existing target → a collapsed summary Card (the monthly target figure large, extra-income sub-line, then a 2-column grid of only the fields that actually have a meaningful value — target date via the existing friendly-date formatter, weekly hours, income type/work mode if not "any", max startup cost) with an "แก้ไข" (Edit) button that reveals the form. `IncomeTargetForm` gained `onSuccess`/`onCancel` props: `onSuccess` fires from a `useEffect` watching the existing `state.success` flag from `useActionState` (no new submit-handling logic, just observing what was already there) and collapses back to the summary; a new "ยกเลิก" (Cancel) button next to Save does the same without submitting. `IncomeTargetSection` simplified to just `<IncomeTargetView target={target} />` — it no longer owns the Card wrapper itself, since the view component now owns both of its own states' framing.

**Verified visually**: temporary preview rendering `IncomeTargetView` twice (once `target={null}`, once with a realistic filled target) + Playwright — confirmed (a) no target renders the open form directly, (b) an existing target renders the collapsed summary with correct formatted values (not raw form-field echoes), (c) clicking "แก้ไข" opens the form pre-filled with the existing values, (d) clicking "ยกเลิก" collapses back to the summary unchanged. Preview route, spec, and the temporary `middleware.ts` public-route addition all removed afterward — confirmed via `git status` that `middleware.ts` matches its committed state exactly.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — no domain logic touched, purely a presentational view/edit toggle), build ✅ (49 routes, `/qa-preview-temp` confirmed absent from the route table).

## Dashboard — Net Worth growth mini chart (2026-09-17)

**Trigger**: the user asked for a chart on the dashboard home screen showing the current net worth's growth over time, and explicitly asked for a design recommendation on how far back it should look — "enough that someone using it for the first time likes it, understands it immediately, and it looks good."

**Time range chosen: 6 months, not 1/3/12+.** Reasoning: 1-3 months rarely shows a visually convincing trend (WEALTH OS is a young product — most real accounts won't have a year of history yet, so a 12-month/all-time default would mostly render as a short line stub against a lot of empty space); 6 months is long enough to read as a real trend line at a glance while staying realistic for how much history an actual early user is likely to have. This mirrors `/money/net-worth`'s existing full history chart in spirit (same Recharts `LineChart` conventions) but on the dashboard it's deliberately a compact, unlabeled "sparkline" — no axes, no grid, just the shape — so it reads in one glance under the headline number instead of competing with it, per `UX_GUIDELINES.md`'s "one card = one idea" and "action before analytics" (`NextBestActionCard` still ranks above it on the page).

**Found and fixed a real data-freshness gap while building this**: `net_worth_snapshots` rows were previously written *only* when a user visited `/money/net-worth` itself (`recordTodaysNetWorthSnapshot()`, called from that page alone). A user who only ever opens the dashboard would never accumulate any history for a dashboard chart to show, no matter how long they used the app. Fixed with a new `ensureTodaysNetWorthSnapshot()` in `src/features/net-worth/queries.ts`, mirroring the existing `ensureTodaysWealthScore()` once-per-day pattern exactly (check the latest stored date first, only write if it isn't today) — called from the dashboard's own data-loading path now too, so history builds up regardless of which page a user actually opens. Also added `getNetWorthSnapshotsSince(monthsBack)` — a proper date-range query (`snapshot_date >= 6 months ago`, ascending), replacing the old fixed-row-count `getNetWorthSnapshots(2)` call in `NetWorthHero` specifically because a row-count limit doesn't reliably mean "last 6 months" for a sparsely-visited history.

**What shipped**: `src/features/dashboard/components/net-worth-mini-chart.tsx` (new, `"use client"`) — a 64px-tall `LineChart` sparkline, `dot={false}`, animated in unless `prefers-reduced-motion` (same convention as every other chart in the app). Slotted into `NetWorthHero` (`src/features/dashboard/components/net-worth-hero.tsx`) directly below the "+฿X · +Y%" change line and above the assets/liabilities grid, with a small "6 เดือนที่ผ่านมา" (last 6 months) label above it — shown only when there are ≥2 data points (matches the existing `noHistoryYet` threshold used elsewhere for this exact data). The hero card flips between a dark "highlight" background (positive net worth) and a plain card (negative) — the chart's stroke color follows: `#7FD6B2` (the same hardcoded mint already used for the positive-delta text on the dark highlight background, chosen for contrast — `var(--color-chart-1)` doesn't read clearly against that same dark green) on highlight, `var(--color-chart-1)` on default, matching every other chart's convention in the app.

**Verified visually**: a temporary preview page rendered `NetWorthMiniChart` directly with three realistic synthetic datasets (upward-trending positive net worth on the highlight card, upward-trending negative net worth on the default card, and a minimal 2-point case) and screenshotted at mobile width — confirmed the line is clearly visible and readable against both card backgrounds, the label reads correctly, and the sparse 2-point case still renders a clean line rather than looking broken. (This checked the new chart's rendering/contrast directly rather than the full live data path, since exercising `ensureTodaysNetWorthSnapshot`/`getNetWorthSnapshotsSince` end-to-end would need a disposable staging login; the query itself reuses the exact same `.gte()` date-range-filter pattern already proven elsewhere in this codebase, e.g. `monthly-review/queries.ts`/`transactions/queries.ts`.) Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — no new domain/financial logic, the chart is a pure presentation of already-existing `net_worth_snapshots` data), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own visual check on their device, per this session's usual "test → commit + push" cadence.
...
4. ~~Consider a small follow-up migration to drop `create_transfer`'s original 6-parameter overload~~ — **written 2026-09-25** as `0020_drop_dormant_create_transfer_overload.sql`, part of the production-hardening pass (see that section below); not yet applied to any database, needs `supabase db push`.
5. ~~`src/features/recurring/actions.ts`'s recurring-transfer confirmation still has no double-submit protection of its own~~ — **already done**, contrary to this line: `confirmRecurringTransaction()` derives `idempotencyKey = deterministicUuid(\`recurring-confirm:${recurring.id}:${recurring.next_due_date}\`)` and passes it through as `p_client_request_id`/`client_request_id`. This "Next Task" line was stale (doc drift, corrected 2026-09-25) — the fix had already shipped without this list being updated.
...
15. **A `min-w-0` fix only works if every ANCESTOR flex box between the text and the overflow point can also shrink — an ancestor with `flex-shrink: 0` (e.g. a Tailwind `shrink-0` class) blocks it completely, regardless of how many `min-w-0`/`truncate` classes are added downstream.** `flex-shrink: 0` disables shrinking outright; it isn't just a floor that `min-width` can override. When customizing a shared UI primitive's children (e.g. putting custom multi-part content inside a component library's `SelectItem`/`MenuItem`/similar), check the primitive's OWN internal wrapper classes for a stray `shrink-0` before assuming a local `min-w-0` fix will work — it won't, and will look like it silently did nothing. Found via the account-picker checkmark-overlap bug above, where `select.tsx`'s shared `ItemText` had exactly this `shrink-0`.

## Transactions — "Has notes" filter, browse every recorded note in one place (2026-09-17)

**Trigger**: after the earlier tap-to-expand notes fix, the user's screenshot showed it working correctly for one transaction (Grab, note "ไปทำงาน") — arrows pointed at both the expanded note and the chevron. They then asked for every note they've ever recorded to have this same detail. Since the chevron already appears generically wherever `transaction.notes` has content (not special-cased to one transaction), the other rows visible in that screenshot (7-eleven, อาหาร, ทีพัก) simply don't have notes saved — not a bug. Asked the user to clarify via `AskUserQuestion` what was actually wanted (the two read as very different scopes: "nothing's broken" vs. "build a real feature"); **they chose "bring every recorded note together in one place."**

**What shipped**: rather than a new parallel page (which would duplicate `TransactionList`/`TransactionRow`, against CLAUDE.md's "never create duplicate modules"), added a **"มีหมายเหตุ" (Has notes) filter** to the existing `/money/transactions` filter bar — the same page already reachable from the dashboard's "ดูทั้งหมด" (see all) link, so it's genuinely "one place," not a new one.
- `getTransactions()` (`src/features/transactions/queries.ts`) gained a `hasNotes` filter — `.not("notes", "is", null).neq("notes", "")` — reusing the exact same `.gte()`-style range-filter idiom already used for `from`/`to` elsewhere in the same function.
- `TransactionFilters` (`transaction-filters.tsx`) gained a pill-style toggle chip (outline when off, filled `bg-primary` when on — same on/off visual language as the rest of the app's active-filter states) below the existing 4-column search/type/account/category grid, wired through the same `setParam()`/URL-search-param mechanism the other filters already use (`?notes=1`).
- **The one genuinely new behavior**: when this filter is active, each `TransactionRow` now renders with its note already expanded (`defaultDetailsOpen`, a new optional prop defaulting to `false` everywhere else) instead of requiring a tap per row — since the entire point of this filtered view is reading notes, requiring an extra tap on every single row would defeat the purpose. Every other list (the dashboard's compact list, the default unfiltered `/money/transactions` view) is completely unaffected — still tap-to-expand, one row at a time, exactly as before.
- `TransactionList`'s `isDefaultView` check (governs which empty-state message shows, and whether Quick Repeat renders) now also treats `hasNotes` as an active filter, so filtering to notes-only and finding zero shows the honest "no results for this filter" message rather than the "you have zero transactions ever" one.

**Verified visually**: a temporary preview rendering `TransactionFilters` and two `TransactionRow`s (one with a Thai note, `defaultDetailsOpen` on) + Playwright — confirmed the chip's inactive (outline) and active (filled) states via two URLs (`?notes=1` vs. none, since the real component always `router.push`es to the real `/money/transactions` route — simulated the URL state directly rather than clicking through, which would have navigated the preview itself away to an auth-gated route), and confirmed both rows render already-expanded with the chevron pre-rotated. Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a query filter and a presentational default-prop, no new domain/financial logic to unit test, consistent with how the existing `accountId`/`categoryId`/`search` filters are also untested at this layer), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own check on their device, per this session's usual "test → commit + push" cadence.

## Fix — Income vs Expense chart's y-axis numbers clipped, and were silently 100x too large (2026-09-17)

**Trigger**: the user's screenshot of the dashboard's "รายรับเทียบรายจ่าย" (Income vs Expense) chart showed the y-axis tick labels visibly cut off at the left edge — leading digits missing from every 6-digit number.

**Root cause #1 (the reported clipping)**: `IncomeVsExpenseChart` (`src/features/dashboard/components/charts.tsx`) gave its `<YAxis>` a fixed `width={40}` — enough for a short number, not for a 6-digit one like "600000" at 12px font, which needs roughly 45-50px. Recharts right-aligns tick text against that reserved width, so text wider than the allocation renders starting to the left of the chart's own SVG boundary and gets clipped there — exactly the "numbers look like they're breaking out of the frame" the user described.

**Root cause #2 (found while reproducing the bug with realistic mock data, not previously reported)**: the chart's `<Bar dataKey="raw">` and therefore its y-axis domain were built directly from the raw **cents** value (`raw: incomeCents`/`expensesCents`, un-divided) — while the tooltip separately calls `formatMoney()`, which does correctly divide by 100 before display. Net effect: hovering a bar showed the correct baht amount, but the axis gridline numbers next to it were literally 100x too large (e.g. an axis reading "600000" next to a bar whose tooltip says "฿6,000"). This is a real, silent financial-display correctness bug, not just cosmetic — caught only because reproducing the clipping fix with realistic amounts made the mismatch between the axis scale and the known real-world expense range obvious.

**Fix, both in the same `<YAxis>`**: added a `tickFormatter` using `Intl.NumberFormat(locale, { notation: "compact" })` (e.g. "600K", "1.2M") wrapped around the existing `centsToNumber()` helper (`src/lib/financial/money.ts`, already used everywhere else in this codebase for cents→baht conversion) — this fixes both problems in one change: compact notation is short enough to never need more than the axis's reserved width again regardless of how large a future amount gets, and running the value through `centsToNumber()` first means the label now shows the same real baht scale the tooltip already showed correctly. Widened `width` from 40 to 44 as a small additional safety margin. No change to the `Bar`'s own `dataKey`/domain or the tooltip — both were already internally consistent (Recharts scales the visual bar height from whatever domain it's given; only the axis *label text* was wrong).

**Verified visually**: a temporary preview rendered the real component at two scales — the reported bug's actual range (~฿495,000) and a much larger one (~฿9.9M, to confirm "M" compact notation also works) — screenshotted before and after the fix. Before: axis read "60M"/"45M"/"30M"/"15M" for a ~฿495,000 expense (the cents-not-divided bug, reproduced first to confirm root cause #2 before fixing it). After: axis correctly reads "600K"/"450K"/"300K"/"150K"/"0", fully visible with room to spare, and the millions-scale case correctly reads "10M"/"7.5M"/"5M"/"2.5M"/"0". Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a chart-label formatting fix, no new domain/financial calculation function to unit test; `centsToNumber()` itself is already covered by existing `money.test.ts`), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).
...
## Fix — editing an account (and 15 other money-amount forms) could silently fail to save (2026-09-17)

**Trigger**: the user's screenshot showed the "แก้ไขบัญชี" (Edit Account) dialog with an opening balance of "3839.1" — clicking "บันทึก" (Save) appeared to do nothing: no error message, no loading state, dialog stayed open.

**Root cause (documented, but not 100%-provable in this sandbox — see "Not fully verified" below)**: `opening_balance`'s `<input type="number" step="0.01">` has a well-documented HTML5 failure mode: the browser's native constraint validation checks whether the entered value is an exact multiple of `step` relative to `min`, and — especially on iOS Safari, and especially inside a standalone/installed PWA (which this app's screenshot appears to be, given the black status bar) — a value that *should* be valid can be silently flagged `stepMismatch`, blocking form submission with **no visible feedback at all**: no native validation bubble renders in that context, `formAction` never fires, and the `useActionState` state (which is what drives the on-screen error message) never even runs. This exactly matches "press save, nothing happens, no error shown."

**Scope widened deliberately**: `type="number" step="0.01"` on a money-amount field turned out to be the standard pattern across the codebase — used in **38 places across 16 forms** (accounts, assets, budget, category budget, debt planner, emergency fund, forecast scenarios, goals, income sources, income target, liabilities, major expenses, money year, quarterly plans, onboarding, recurring transactions). The one form that does NOT use this pattern, `transaction-form.tsx`, deliberately uses a custom text-input + hidden-field approach instead (built and proven earlier this session, and long before that) — meaning this exact defect class was already avoided in exactly one place already, just never applied everywhere else. Fixed all 38 occurrences the same way rather than leaving 37 of them exposed to the identical silent-failure risk.

**Fix**: `step="0.01"` → `step="any"` everywhere (`sed`-applied across all 16 files, then spot-checked). `step="any"` is the documented, official HTML5 way to disable the step-multiple constraint entirely while keeping `type="number"`'s numeric-keyboard UX and every other constraint (`required`, `min`) intact — it's a strict removal of one specific check, not a broader change. No server-side change needed: every one of these fields was already validated for finiteness/precision by its Zod schema (`z.coerce.number().finite(...)`) independent of the HTML `step` attribute, so nothing about server-side correctness depends on the browser's own step check.

**Verification and its limits — stated plainly**: this sandbox cannot launch a WebKit/Safari browser at all (`browserType.launch` fails for every `iphone-*` Playwright project here, a standing limitation noted earlier this session too), so the exact iOS failure could not be reproduced or re-confirmed fixed directly. What WAS verified: (1) a live Chromium check confirming the field's `step` attribute is now genuinely `"any"` and `checkValidity()`/`stepMismatch` are clean for the reported value; (2) a full signup-based end-to-end attempt against staging was tried but blocked by this environment having no way to complete the resulting email-confirmation step (and then an email-send rate limit on retries) — so the complete real save flow was not exercised live this session. This fix is shipped on the strength of a well-documented, standard root cause and a change that is provably safe (removes one specific constraint, changes nothing else) rather than a live confirmed-fixed screenshot. **Next real confirmation should be the user re-testing this exact edit-account flow on their own iPhone.**

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — an HTML attribute change, no logic touched), build ✅ (49 routes).

**Not yet committed** — pending the user's own check on their device.

## Copy tweak — account name placeholder example (2026-09-17)

User asked to change the "ชื่อบัญชี" (Account name) field's placeholder example from a specific bank name ("ธนาคารไทยพาณิชย์") to a generic account-type label ("บัญชีหมุนเวียน"). Pure i18n string change (`accounts.namePlaceholder`, `th.json` only — `en.json`'s "SCB" example left as-is, not requested). No logic touched; typecheck ✅.

**Not yet committed** — pending the user's own check.

## Fix — the REAL cause of "save doesn't close the dialog": a stale-boolean useEffect dependency bug in 17 forms (2026-09-17)

**Trigger**: after the `step="0.01"`→`step="any"` fix above (still unverified/unconfirmed by the user), the user tested again and reported more precisely this time: entering data and saving the "Add Account" dialog **did** save the data (the new account appeared in the list, visible behind the still-open dialog) — but the dialog itself never closed. This is a materially different, much more specific symptom than "nothing happens," and pointed away from the step-validation theory (a blocked submission would mean the data never saves at all) toward something in the post-success UI logic instead.

**Root cause, found by reading the page tree, not guessed**: `/money/accounts`'s "+ เพิ่มบัญชี" button is a single, page-level `<AccountForm />` instance (`account-list.tsx`) that stays mounted for the whole page session — every account a user adds goes through the exact same component instance, not a fresh one each time. That component's close-on-success logic was:
```tsx
useEffect(() => {
  if (state?.success) setDialogOpen(false);
}, [state?.success]);
```
`useActionState` returns a **new** `{ success: true }` object every time an action completes — but `state?.success` is a primitive `boolean`, and React's effect dependency comparison uses `Object.is`. On the *first* save, `state?.success` goes `undefined → true`: a real change, so the effect fires and the dialog closes correctly. On any *second* save through the same instance, it goes `true → true` — structurally identical, so **React never re-runs the effect**, and `setDialogOpen(false)` is never called again. The bug is invisible on a form's first-ever use and only appears from the second successful save onward through the same mounted instance — exactly matching a user who had already successfully added one account, then hit this on the next one.

**Verified by reproducing the mechanism in isolation, not just by reading the code**: built a temporary preview with two intentionally near-identical components — one with the buggy `[state?.success]` dependency, one with the fix (`[state]`, the whole object instead of just its field) — both wired to a fake action (`{success:true}`, no real backend needed) so the exact React scheduling behavior could be observed directly. Playwright drove each through two successive open→fill→save cycles: **the buggy version closed correctly after save #1 but stayed open after save #2 (screenshotted, dialog still showing "submit #2" in its title) — the fixed version closed correctly both times.** This is the same class of confirmation this session used for the sticky-scroll and account-picker bugs earlier — reproduce the actual mechanism, don't just theorize about it.

**Scope**: the identical `[state?.success]` dependency existed in **17 files** — every dialog-based form in the app (accounts, assets, budget, category budget, emergency fund, forecast scenarios, goals, income sources, income target, liabilities, major expenses, money year, quarterly plans, recurring transactions, skills) plus, notably, **`transaction-form.tsx` and `transfer-form.tsx`** — the two most-used forms in the whole app. Fixed all 17 the same way: `[state?.success]` → `[state]`.

**A second, more serious bug this same defect caused, found while fixing `transfer-form.tsx`**: that form's effect doesn't just show a toast — it also rotates `client_request_id` (`setClientRequestId(crypto.randomUUID())`), the idempotency key described in CLAUDE.md's "Transaction Idempotency" section, specifically so each new transfer submits under a fresh key. With the same stale-dependency bug, **a second transfer through the same open form instance would never rotate its key** — meaning it would resubmit under the FIRST transfer's already-used `client_request_id`, and the server's idempotency logic would then correctly-but-wrongly treat it as a retry of the first transfer and silently no-op it, **never actually saving the second transfer at all**. This is a real, financially-significant bug (a user's second transfer of a session could vanish with no error), not just a UI polish issue — now fixed by the same one-line-per-file change.

**Also fixed as the same defect class**: `income-target-form.tsx`'s `onSuccess?.()` call (built earlier this session, for the view/edit toggle) had the identical bug — editing the Income Target a second time in the same page session would save correctly but fail to collapse back to the summary view. `emergency-fund-form.tsx`'s `window.scrollTo({top:0})` on success had it too (a missed scroll on a repeat save, lower severity but the same root cause).

Quality gate: lint ✅ (all `eslint-disable-next-line react-hooks/exhaustive-deps` comments left in place — still needed for other, unrelated omitted deps like setter functions/props, confirmed by a clean lint pass, not just assumed), typecheck ✅, tests ✅ (525/525, unchanged — this bug lives entirely in `useEffect` scheduling behavior, which this codebase has no jsdom/React Testing Library setup to unit-test; verified instead via the live Playwright reproduction described above), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**This is very likely the actual, complete explanation for both of the user's last two reports** — the earlier `step="0.01"` fix remains in place as a legitimate, independent safety improvement (still worth keeping), but this dependency-array bug is the one that was empirically reproduced end-to-end and directly matches what the user described and saw.

**Not yet committed** — pending the user's own check on their device (this time, testing specifically: add or edit two things in a row through the same button, not just one).

## Fix — expanding the category list inside a dialog made the whole dialog unscrollable (2026-09-17)

**Trigger**: the user reported clicking "ดูทั้งหมด" (the Category Picker's "show all categories" chip, inside the Add Recurring Transaction dialog) and then being unable to scroll the screen up or down at all — confirmed via two screenshots: the second one showed the category list fully expanded (many more categories, "สมาชิก/Subscription" selected, a "แสดงน้อยลง" collapse button now visible), with the rest of the form (merchant, notes, frequency, dates, Save button) below the fold and unreachable.

**Root cause**: `DialogContent` (`src/components/ui/dialog.tsx`, the shared primitive behind every "Add/Edit X" dialog in the app — accounts, assets, goals, liabilities, recurring transactions, etc.) had **no `max-height` or `overflow-y-auto`** — just centered `fixed` positioning with no height constraint at all. For a short form this never mattered (it fits the viewport and nothing needs to scroll). But `CategoryPicker`'s "ดูทั้งหมด" button (`category-picker.tsx`) reveals every category inline on click — for this app's real category list, that can add a dozen-plus extra chips, making the dialog's natural content height taller than the screen. Since `position: fixed` content doesn't scroll with the page, and the dialog itself had no internal scroll container, everything past the viewport edge (top or bottom) became **permanently unreachable** — not just requiring a scroll, genuinely inaccessible by any means. This is a structural bug in the shared dialog primitive, not specific to the recurring-transaction form — any dialog whose content can grow (a long validation error, a big picker, a future feature) was equally exposed.

**Fix**: added `max-h-[calc(100dvh-2rem)] overflow-y-auto` to `DialogContent`'s className — mirrors the existing `max-w-[calc(100%-2rem)]` width-margin convention already used on the same element. `dvh` (dynamic viewport height) rather than `vh`, specifically so a mobile browser's address-bar show/hide doesn't clip the dialog. This is a one-line change to a single shared primitive that fixes the issue for every dialog in the app at once, not just this one form.

**Verified by reproducing the exact failure first, then confirming the fix resolves it — including a negative control**: a temporary preview rendered the real `RecurringTransactionForm` with a realistic ~19-category mock list at a short (390×700) viewport, matching the report's proportions. With the fix reverted (`git stash` on just `dialog.tsx`), Playwright confirmed the Save button sat at y=845 — 145px below the 700px-tall viewport, unreachable no matter how the page was scrolled (matches "เลื่อนหน้าจอขึ้นลงไม่ได้" exactly). With the fix restored, the identical steps land Save at y=636, fully on-screen and scrollable into view normally — screenshotted both states.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a CSS-only change to a shared UI primitive), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own check on their device. This closes out today's chain of "form feels broken" reports: the step="any" fix, the `[state]` dependency-array fix, and this scroll fix likely together account for everything reported across the last several messages.

## Fix — start/end date fields could visually overlap on a real device (2026-09-17)

**Trigger**: the user's screenshot (with a red arrow) pointed at the "วันที่เริ่ม" / "วันที่สิ้นสุด (ถ้ามี)" (start/end date) row on the Recurring Transaction form, asking whether the two boxes were overlapping.

**Root cause**: same defect class as the account-picker checkmark-overlap bug fixed earlier this session (carried-forward lesson #14/#15) — the two date inputs sit in a `grid grid-cols-2 gap-4` with no `min-w-0` on either grid item. A native `<input type="date">`'s rendered width isn't fully CSS-controlled: **iOS Safari renders the value using the device's own locale**, and a Thai Buddhist-calendar date like "15 ต.ค. 2569" (the screenshot's actual filled value) is visibly wider than a numeric "15/10/2026" — wide enough that, without `min-w-0`, the grid track can't shrink to fit and the filled `start_date` box can push into `end_date`'s column. This is the only place in the app pairing two date inputs side by side in a 2-column grid (checked every other `type="date"` usage — `forecast-scenario-form.tsx`, `goal-form.tsx`, `liability-form.tsx`, `major-expense-form.tsx`, `target-date-field.tsx`, `date-field.tsx` — none pair two dates in one row), so the fix is scoped to this one file.

**Fix**: added `min-w-0` to both grid item wrappers in `recurring-transaction-form.tsx`'s date row — the `Input` component itself already had `w-full min-w-0` (confirmed by reading `input.tsx`), so the missing piece was purely the grid item's own implicit `min-width: auto`.

**Verification and its honest limits**: this sandbox still cannot launch WebKit (same standing limitation as the `step="0.01"` fix earlier today), so the exact Thai-locale-date rendering width on real iOS Safari couldn't be directly screenshotted. What was verified: a live Chromium reproduction of the real form with a filled `start_date` at a 390px viewport (Chromium's own date input renders compactly regardless of locale, so it never showed the overlap itself — expected, not a gap in the fix) confirmed no regression, and a direct computed-style check confirmed `getComputedStyle(...).minWidth` on the grid item is genuinely `"0px"` after the fix (i.e. the CSS mechanism that prevents the overflow is actually in effect, not just present in source and silently overridden by something else).

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a CSS class addition, no logic touched), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own check on their device (this bug can only be truly confirmed fixed on the real iPhone that showed it, same caveat as the earlier `step="any"` fix).

## Recurring transaction cards show their own description instead of a generic line (2026-09-17)

**Trigger**: the user's screenshots showed the recurring transactions list — each card's subtitle read a generic "รายจ่าย · รายเดือน · กำหนดครั้งถัดไป: 2..." (Expense · Monthly · Next due: 2...) truncated line, even though every one of these recurring transactions already had a specific, useful note typed into its own "รายละเอียด" (description) field when created (e.g. "Subscription chatgpt ที่ปรึกษาโปรเจคและเรื่องส่วนตัว"). Asked for the card to show that description instead.

**Fix**: `RecurringTransactionCard` (`recurring-transaction-card.tsx`) now shows `recurring.description` as the subtitle when it has content — the generic type/frequency/next-due line is kept as a fallback only for recurring transactions that don't have a description set, so nothing regresses for existing entries without one. One-line conditional change, no new field/migration needed (`description` already existed and was already being collected by the form, just never displayed here).

**Verified visually**: a temporary preview rendered two cards side by side — one with a description (confirmed it displays the description text, truncated the same way the old line was) and one without (confirmed it still falls back to the original type/frequency/next-due line, unchanged). Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a display-only change), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own check on their device.

## "Other"-type asset cards show a smarter icon than a generic building (2026-09-17)

**Trigger**: the user's screenshot showed an asset named "โน๊ตบุ้ค hp - victus" (a laptop) rendered with the generic building icon, since there's no dedicated `AssetType` for electronics/gadgets — everything not fitting the existing 9 types (cash/bank/savings/investment/gold/crypto/property/vehicle/business) falls under "other," which always showed the same `Building2` icon regardless of what the item actually was.

**Design decision — keyword inference over a new AssetType**: adding a real `"electronics"` asset type would need a database migration (the `asset_type` column is a plain CHECK constraint, not an enum, so technically easy) plus this session has no DDL-capable credential to apply it, meaning the feature wouldn't actually work until the user separately ran a migration by hand — real friction for what is purely a cosmetic icon choice. Instead, `AssetCard` now infers a more specific icon from the asset's own name when `asset_type === "other"` (laptop/notebook, phone, camera, watch — the most common "other" gadgets), falling back to the generic building icon exactly as before when nothing matches. `asset_type` itself, the database, and every calculation that reads it are completely untouched — purely a display-layer icon choice.

**A real bug caught during verification, not assumed away**: the first implementation used an explicit list of spelled-out Thai keywords ("โน้ตบุ๊ค", "โน๊ตบุ๊ค") for "notebook" — visually verifying against the user's *exact* reported name ("โน๊ตบุ้ค") still showed the generic building icon, because Thai loanwords like "notebook" have no single standardized spelling: the tone marks (ไม้โท/ไม้ตรี) can land on either syllable, giving 4 equally-common variants (โน้ตบุ๊ค/โน๊ตบุ๊ค/โน้ตบุ้ค/โน๊ตบุ้ค), and the keyword list only covered 2 of them. Fixed properly rather than just adding more variants to the list: `matchOtherAssetIconKey()` now strips Thai tone-mark combining characters (`่`-`๋`) from both the asset name and the keyword before comparing, normalizing every spelling variant to the same base form — a general fix instead of an ever-growing enumeration.

**A lint finding worth noting for future icon-picking code**: the first version computed the icon via `asset.asset_type === "other" ? inferOtherAssetIcon(asset.name) : ASSET_ICONS[...]` (a function call feeding directly into the JSX-rendered variable) — `eslint-plugin-react-hooks`'s `static-components` rule flagged this as "component created during render," even though the function is pure and only ever returns one of a small set of already-stable top-level icon references. Restructured so the *keyword matching* returns a plain string key (not a component), and the actual icon is selected via a plain object-index expression (`OTHER_ASSET_ICONS[otherIconKey]`) right at the JSX-feeding assignment — the lint rule accepts a `Record` lookup as provably stable but not a function-call result, so keeping the final component selection as a plain lookup (with any inference logic happening one step earlier, on a string) is the pattern to reuse if this comes up again elsewhere.

**Verified visually**: a temporary preview rendered six assets — the exact reported name plus "iPhone 15 Pro," "กล้อง Sony A7," "นาฬิกา Rolex," an unmatched "เฟอร์นิเจอร์บ้าน" (furniture, correctly still generic), and an existing "vehicle"-type asset (confirmed unaffected) — screenshotted before and after the tone-mark fix. Before: the reported name still showed the building icon (reproducing the gap). After: laptop, phone, camera, and watch icons all rendered correctly, furniture correctly stayed generic, and the untouched vehicle type was unaffected. Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅ (confirmed clean only after the static-components restructure — not assumed), typecheck ✅, tests ✅ (525/525 — no dedicated unit test added for this: it's a cosmetic icon heuristic, not financial-domain logic, verified instead via the live visual check described above, consistent with how other presentational-only changes were handled this session), build ✅ (49 routes, `/qa-preview-temp` confirmed absent).

**Not yet committed** — pending the user's own check on their device.

## Fix — AI Money Coach chat showed raw "**" markdown syntax instead of rendering it (2026-09-17)

**Trigger**: the user's screenshot showed the AI's reply with literal `**text**` asterisks visible around emphasized words ("**web development**", "**รับพัฒนาเว็บไซต์ฟรีแลนซ์**") instead of bold text — "ไม่ชอบ ** เลย เอาออกให้หน่อย" (don't like the ** at all, take it out).

**Root cause**: `AICoachChat` (`ai-coach-chat.tsx`) rendered every message as plain text (`renderBubbleParagraphs` split on blank lines and dropped the raw string straight into a `whitespace-pre-wrap` div) — but the underlying AI model, like essentially every modern chat model, naturally writes markdown (bold, numbered lists) when structuring a comparative answer such as ranking income opportunities by match score. With no markdown renderer in the chat UI, that syntax showed up as literal characters instead of formatting.

**Fix**: added `react-markdown` + `remark-breaks` (new dependencies — `remark-breaks` specifically so a single newline within a paragraph still renders as a line break, matching the old `whitespace-pre-wrap` visual behavior, since plain CommonMark only treats a *blank* line as a paragraph break). Deliberately minimal `components` overrides — only `p`/`strong`/`ul`/`ol`/`li`/`a`, restyled to fit the compact chat bubble instead of the browser's default block spacing; no headings, tables, images, or raw HTML (`react-markdown` never renders raw HTML by default, so this is also inherently safe against any injected markup, not just "not implemented yet"). The existing per-paragraph `Bubble` splitting (`text.split("\n\n")`, one bubble per blank-line-separated block) is unchanged — only how the text *inside* each bubble renders changed.

**Scope check**: confirmed this is the only surface in the app that renders live LLM prose — every other AI-adjacent component (insights, health check) renders deterministic, pre-computed data per CLAUDE.md's "Financial Logic must be deterministic" rule, never raw model text, so none of them had this gap.

**Verified visually**: a temporary preview reused the exact same `Bubble`/`ReactMarkdown`/`remarkBreaks` setup with the user's own reported message text (bold skill name, a 3-item numbered list with bold titles) — confirmed via both a screenshot and an automated check that the rendered page contains zero literal `**` characters, `"web development"` renders inside a real `<strong>` element, and the numbered list produces exactly 3 real `<li>` elements. Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status`.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a rendering-layer change, no logic touched), build ✅ (49 routes).

**Not yet committed** — pending the user's own check on their device.

## User guide — in-app `/help` page + standalone Artifact (2026-09-17)

**Trigger**: the user asked for an easy-to-read usage guide for WEALTH OS, wanting it in both places — a standalone shareable document and a real page inside the app — and asked that it be kept up to date every time a new feature ships (a standing instruction going forward, not a one-time request).

**What shipped**:

1. **In-app `/help` page** (`src/app/(app)/help/page.tsx`) — a real authenticated route using the app's own `Card` design system, covering: the Track→Analyze→Plan→Earn→Grow philosophy strip, a 3-step "getting started" walkthrough, one Card per nav section (หน้าแรก/การเงิน/วางแผน/หารายได้/AI/ภารกิจ) listing every real shipped feature with a one-line plain-language description and a Plus/Pro or Free-limited badge where relevant, a glossary Card (Net Worth, Wealth Score, Safe-to-Spend, Emergency Fund, Income Gap, Money Year), and the standing "never promises you'll get rich" disclaimer. Reachable from `/profile` via a new row matching the existing Billing-summary row's exact pattern (icon + label + chevron, `Button variant="ghost"` wrapping a `Link`). Fully bilingual — all copy lives in a new `help` i18n namespace in both `th.json` and `en.json` (arrays of `{title, description}` objects per section, matching the existing `billing.pricing.featureList` convention for bulk content), so the Dictionary type stays the single source of truth.

2. **Standalone Artifact** (published, private — link given to the user directly, not stored in the repo) — the same content, restyled as a real designed page: WEALTH OS's own actual brand tokens pulled from `globals.css` (forest green `#1f4d3e`/`#2d6b52`, IBM Plex Sans Thai — the exact font the app itself loads via `next/font`), a sticky scroll-spy section nav, the philosophy flow as a horizontal strip, card grids per section, a glossary, and a 3-column Free/Plus/Pro pricing comparison. Both light and dark themes implemented per the artifact-design token pattern.

**Content accuracy — not guessed**: before writing either version, delegated a research pass (`Explore` subagent) across `PROJECT_STATUS.md`'s "Working" section, every route under `src/app/(app)/`, `th.json`'s top-level namespaces (as a completeness cross-check), `src/lib/billing/plans.ts`, and CLAUDE.md's own definitions of Net Worth/Wealth Score/Safe-to-Spend/etc. — so every feature and every plan-tier claim in the guide matches what's actually shipped today, not what's planned or assumed.

**Verified visually**: the in-app page can't be reached by a plain `curl`/Playwright check without a real session (the `(app)` layout itself — not just middleware — redirects to `/login` when there's no profile), so a temporary preview page rendered the real `HelpPage` component directly (it doesn't redirect on its own; only the shared layout does) to screenshot it standalone. Confirmed all 6 feature-section Cards, the getting-started steps, badges, and the glossary render correctly and match the app's existing Card/Badge visual language. Preview route and spec deleted afterward, `middleware.ts` confirmed reverted via `git status` (an earlier attempt to make `/help` itself temporarily public for testing was reverted once it became clear the `(app)` layout's own auth check made that insufficient anyway).

**Standing instruction for future sessions**: keep both the in-app `/help` page's `help` i18n content and the standalone Artifact in sync whenever a genuinely new user-facing feature ships — this was explicitly asked to be an ongoing maintenance habit, not a one-time build.

**Follow-up (same day)**: the user asked for illustrations/graphics/emoji and better layout, specifically so a brand-new WEALTH OS user could understand it "as fast and easily as possible." Added an `emoji` field to every section, feature item, getting-started step, and glossary entry in the `help` i18n namespace (both locales) — rendered as a leading icon-chip (a small rounded circle, mirroring the exact `AccountCard`/`AssetCard` per-item icon convention already used elsewhere in the app) in the in-app page, and as a matching chip in the Artifact. Also added the app's own `WelcomeIllustration` (`@/components/illustrations`) as a friendly hero image at the top of the in-app page, and a hand-translated inline-SVG copy of that same illustration (using the Artifact's own CSS custom properties instead of the app's, since the Artifact is a standalone document with no access to `globals.css`) as the Artifact's hero. Replaced the Artifact's decorative `01/02/03...` numeric section kickers with the same section emoji — per the artifact-design guidance that structural devices should encode something real, and a section index number wasn't actually meaningful here (unlike the getting-started steps, which kept their numbered circles since that IS a real sequence). Re-verified the in-app page visually again after this pass (same temporary-preview technique as before) — all emoji chips, the hero illustration, and existing badges render correctly together with no layout regressions.

Quality gate: lint ✅, typecheck ✅, tests ✅ (525/525, unchanged — a new content page, no domain logic), build ✅ (50 routes, new `/help` confirmed in the route table, `/qa-preview-temp` confirmed absent).
