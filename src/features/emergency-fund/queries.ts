...
/** Wrapped in React's `cache()` (perf audit finding) — called ~4x independently on a single dashboard render. */
 ...
 * Wrapped in React's `cache()` (perf audit finding) — called ~4x
 * independently on a single dashboard render.
 */
export const getEssentialMonthlyExpenses = cache(async (): Promise<{ cents: number; hasData: boolean }> => {
