...
export function RecurringTransactionForm({ recurring, accounts, categories, trigger, open, onOpenChange }: RecurringTransactionFormProps) {
  ...
  const formId = recurring ? `recurring-transaction-form-edit-${recurring.id}` : "recurring-transaction-form-add";
  const title = recurring ? t("recurring.editRecurring") : t("recurring.addRecurring");
